...
def originHandle():
    with open('./renmin_new.txt', 'r') as inp, open('./renmin2.txt', 'w') as outp:
        ...
        for line in inp.readlines():
            ...
            while i < len(line) - 1:
                ...
                if tag == 'nr' or tag == 'ns' or tag == 'nt':
                    outp.write(word[0] + "\tB-" + map[tag] + "\n")
                    for j in word[1:len(word) - 1]:
                        if j != ' ':
                            outp.write(j + "\tI-" + map[tag] + "\n")
                    outp.write(word[-1] + "\tE-" + map[tag] + "\n")
                else:
                    for wor in word:
                        outp.write(wor + '\tO\n')
                # if tag == 'nr' or tag == 'ns' or tag == 'nt':
